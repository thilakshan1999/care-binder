...
@objc class AppDelegate: FlutterAppDelegate {
    ...
    private let groupId = "group.com.hinetics.carebinder.shared"
    ...
    override func application(
        ...
        // ✅ App Group Token Channel ONLY
        let channel = FlutterMethodChannel(
            name: "app.group.share",
            binaryMessenger: controller.binaryMessenger
        )
        ...
        channel.setMethodCallHandler { [weak self] call, result in
            ...
            case "saveToAppGroup":
                guard let args = call.arguments as? [String: Any],
                    ...
                    result(FlutterError(code: "INVALID_ARGS", message: "Missing tokens", details: nil))
                    return
                }
                ...
                print("✅ Credentials saved to App Group")
                result("success")
            ...
            case "getRefreshToken":
                let defaults = UserDefaults(suiteName: self.groupId)
                let token = defaults?.string(forKey: "refresh_token")
                result(token)
